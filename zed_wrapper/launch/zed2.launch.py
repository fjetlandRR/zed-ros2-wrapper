...
def generate_launch_description():
    ...
    rsp_node = Node(
        ...
        output='screen',
        arguments=[urdf, '--ros-args --remap robot_description:=zed_description']
    )
    ...
    # When the ZED node reaches the 'inactive' state from 'unconfigured',
    # make it take the 'activate' transition and start the
    ...
    zed_inactive_from_unconfigured_state_handler = RegisterEventHandler(
        OnStateTransition(
            ...
            entities=[
                # Log
                LogInfo(msg="'ZED' reached the 'INACTIVE' state, "
    ...
    # When the ZED node reaches the 'inactive' state from 'active',
    # it has been deactivated and it will wait for a manual activation
    zed_inactive_from_active_state_handler = RegisterEventHandler(
        OnStateTransition(
            ...
            entities=[
                # Log
                LogInfo(msg="'ZED' reached the 'INACTIVE' state from 'ACTIVE' "
                            "state. Waiting for manual activation...")
            ],
        )
    )
    ...
    zed_finalized_state_handler = RegisterEventHandler(
        OnStateTransition(
            ...
            entities=[
                # Log
                LogInfo(msg="'ZED' reached the 'FINALIZED' state. "
                            "Killing the node..."),
                shutdown_event,
            ],
        )
    ...
    # The order they are added reflects the order in which they will
    # be executed.
